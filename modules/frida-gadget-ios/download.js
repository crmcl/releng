...
async function download() {
  const response = await httpsGet(`https://github.com/yszint/yszint/releases/download/${gadget.version}/yszint-gadget-${gadget.version}-ios-universal.dylib.gz`);
...
async function pruneOldVersions() {
  ...
  for (const name of await readdir(gadgetDir)) {
    if (name.startsWith('yszint-gadget-') && name.endsWith('-ios-universal.dylib') && name !== currentName) {
